...
public class PlayerDataPacket extends AbstractNetworkPacket{
    ...
    public boolean read(ByteBuffer byteBuffer, NetData config, int version) throws IOException {
        ...
        if (version >= 1) {
            config.isForced = byteBuffer.get() != 0x00;
            config.speed = byteBuffer.getFloat();
            config.isMirrored = byteBuffer.get() != 0x00;
        }
        return true;
    }
    ...
    public void write(ByteBuffer byteBuffer, NetData config) throws IOException {
        ...
        byteBuffer.putFloat(config.speed);
        byteBuffer.put(config.isMirrored ? (byte) 0x01 : (byte) 0x00);
    }
    ...
    public int calculateSize(NetData config) {
        return 22;//1 UUID = 2 Long = 2*8 bytes = 16 bytes + 1 byte for forced flag
    }
}
