...
public abstract class AbstractServerEmotePlay<P> extends ServerEmoteAPI {
    ...
    protected void handleStreamEmote(NetData data, P player, INetworkInstance instance) throws IOException {
        ...
        streamEmote(data, player, false, 1.0f, false, true);
    }
     ...
     */
    protected void streamEmote(NetData data, P player, boolean isForced, float speed, boolean isMirrored, boolean isFromPlayer) {
    ...
    @Override
    protected void setPlayerPlayingEmoteImpl(UUID player, @Nullable KeyframeAnimation emoteData, boolean isForced, float speed, boolean isMirrored) {
        if (emoteData != null) {
            streamEmote(new EmotePacket.Builder().configureToStreamEmote(emoteData).build().data, getPlayerFromUUID(player), isForced, speed, isMirrored, false);
        } else {
