...
public class ClientEmotePlay extends ClientEmoteAPI {
    ...
    public static boolean clientStartLocalEmote(KeyframeAnimation emote) {
        ...
        EmoteInstance.instance.getClientMethods().getMainPlayer().playEmote(emote, 0, false, 1, false);
        return true;
    }
        ...
        switch (Objects.requireNonNull(data.purpose)) {
            case STREAM:
                ...
                if(data.valid || !(((ClientConfig)EmoteInstance.config).alwaysValidate.get() || !networkInstance.safeProxy())) {
                    receivePlayPacket(data.emoteData, data.player, data.tick, data.isForced, data.speed, data.isMirrored);
                }
                break;
            case STOP:
    ...
    static void receivePlayPacket(KeyframeAnimation emoteData, UUID player, int tick, boolean isForced, float speed, boolean isMirrored) {
        ...
        if(isEmoteAllowed(emoteData, player)) {
            ...
            if (playerEntity != null) {
                ...
                playerEntity.playEmote(emoteData, tick, isForced, speed, isMirrored);
            }
            else {
